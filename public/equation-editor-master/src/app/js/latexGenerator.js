var generateLatex = function(expr) {
    ...
    if(expr.hasOwnProperty("operands")) {
        for (var i = 0; i < expr.operands.topLevelContainer.length; i++) {
            var wrapper = expr.operands.topLevelContainer[i];
            console.log(wrapper)
            ...
            switch (wrapper.type) {
                ...
                case "Superscript":
                    ...
                    console.log("som za superscriptToLatex");
                    var endBraces = '';
        ...
        for (var k = 0; k < expr.length; k++) {
            var newWrapper = expr[k];
            console.log(newWrapper)
            console.log(newWrapper.type);
            console.log("som dalej");
            console.log(newWrapper)
            switch (newWrapper.type) {
                case "Symbol":
                    latexString += symbolToLatex(newWrapper);
                    break;
                case "BigOperator":
                    latexString += bigOperatorToLatex(newWrapper);
                    break;
                case "Function":
                    latexString += functionToLatex(newWrapper);
                    break;
                case "Bracket":
                    latexString += bracketToLatex(newWrapper);
                    break;
                case "Operator":
                    latexString += operatorToLatex(newWrapper);
                    break;
                case "BracketPair":
                    latexString += bracketPairToLatex(newWrapper);
                    break;
                case "Integral":
                    latexString += integralToLatex(newWrapper);
                    break;
                case "Accent":
                    latexString += accentToLatex(newWrapper);
                    break;
                case "FunctionLower":
                    latexString += functionLowerToLatex(newWrapper);
                    break;
                case "Limit":
                    latexString += limitToLatex(newWrapper);
                    break;
                case "LogLower":
                    latexString += logLowerToLatex(newWrapper);
                    break;
                case "Matrix":
                    latexString += matrixToLatex(newWrapper);
                    break;
                case "NthRoot":
                    latexString += nthRootToLatex(newWrapper);
                    break;
                case "SquareRoot":
                    latexString += squareRootToLatex(newWrapper);
                    break;
                case "StackedFraction":
                    latexString += stackedFractionToLatex(newWrapper);
                    break;
                case "Superscript":
                    latexString += superscriptToLatex(newWrapper);
                    console.log("som za superscriptToLatex");
                    var endBraces = '';
                    while (typeof expr[k + 1] !== "undefined" && expr[k + 1].type === "Superscript") {
                        k++;
                        latexString = latexString.substring(0, latexString.length - 1);
                        endBraces += '}';
                        latexString += superscriptToLatex(expr[k]);
                    }
                    latexString += endBraces;
                    break;
                case "Subscript":
                    latexString += subscriptToLatex(newWrapper);
                    var endBraces = '';
                    while (typeof expr[k + 1] !== "undefined" && expr[k + 1].type === "Subscript") {
                        k++;
                        latexString = latexString.substring(0, latexString.length - 1);
                        endBraces += '}';
                        latexString += subscriptToLatex(expr[k]);
                    }
                    latexString += endBraces;
                    break;
                case "SuperscriptAndSubscript":
                    var superscripts = [];
                    var subscripts = [];
                    superscripts.push(newWrapper.operands.superscript);
                    subscripts.push(newWrapper.operands.subscript);
                    while (typeof expr[k + 1] !== "undefined" && expr[k + 1].type === "SuperscriptAndSubscript") {
                        k++;
                        superscripts.push(expr[k].operands.superscript);
                        subscripts.push(expr[k].operands.subscript);
                    }
                    while (typeof expr[k + 1] !== "undefined" && expr[k + 1].type === "Superscript") {
                        k++;
                        superscripts.push(expr[k].operands.superscript);
